...
#' @return a vector of coefficients from the final fitted model with top features
#' @author Vedant Mane
#' @details
#' This function runs a univariate regression of y on each predictor in x and calculate a p-value indicating the significance of the association.
...
topten <- function(x, y) {
      p <- ncol(x)
      if(p < 10){
            stop("There are less than 10 predictors in the Dataset")
      }
      pvalues <- numeric(p)
      for (i in seq_len(p)) {
            fit <- lm(y ~ x[,i])
